...
Algorithmic OB trading strategy for BTC/USDT — walk-forward validated, volatility-filtered, outperforming buy-and-hold by +29%.
...
**Capital:** $10,000 | **Fees:** 0.3% round-trip | **Filter:** ATR% 0.8–2.5 | **Position:** 50% fixed
...
| Profit Factor | 1.26 |
| Total Trades | 44 |
| Win Rate | 45.5% (20W / 24L) |
| Avg Win | $227.04 |
| Avg Loss | -$149.96 |
| Win/Loss R | 1.51R |
| Expectancy | $21.40/trade |
| Kelly Criterion | 9.4% |
| Avg Duration | 5.8 days |
| Market Exposure | 71.9% |

### Monthly Returns

| Metric | Value |
|---|---|
| Positive Months | 5/16 (31%) |
| Best Month | +6.65% |
| Worst Month | -6.56% |
| Monthly Sharpe | 0.36 |
...
### Walk-Forward (9 windows, 60d train / 30d test)

| Metric | Value |
|---|---|
| Compounded | **+7.66%** |
| Avg Test Sharpe | 1.10 |
| Overfit Gap | 0.05 |
| Beats B&H | 5/9 windows |

### Statistical Significance

- t-stat: **6.80** (>2.0 threshold) — statistically significant
- 44 trades (below 100 minimum, but walk-forward mitigates)

### Filters Tested

| Filter | Result |
|---|---|
| Vol filter (ATR% 0.8–2.5) | **Winner** — +7.79% single, +7.66% WF |
| HMM regime | Failed — -17.18% WF |
| EWMA vol sizing | Failed — -2.54% WF |
| ADX filter | Hurts performance |
| HTF trend | Hurts performance |
| Time-of-day | Hurts performance |
...
5. Enter on retest with SL beyond OB + TP at 2.0R
...
RegimeFilteredOB(
    ...
    sl_atr_mult=2.0,         # SL = 2x ATR from OB edge
...
The final walk-forward validated strategy. Extends base OB with ATR% regime filter. Used for all benchmark results above.
```python
from pine_ob_strategy import RegimeFilteredOB
strategy = RegimeFilteredOB(use_vol_filter=True, min_atr_pct=0.8, max_atr_pct=2.5)
```
...
9-window walk-forward validation (60-day train / 30-day test):

- **Compounded return:** +7.66%
- **Average test Sharpe:** 1.10
- **Overfit gap:** 0.05 (minimal overfitting)
- **Beats buy-and-hold:** 5/9 windows
