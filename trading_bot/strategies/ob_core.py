"""
Order Block Core - Base Strategy & Types
=========================================
Contains shared types (OBState, OrderBlock) and the base OrderBlockStrategy.
"""
...
class OrderBlockStrategy(Strategy):
    ...
    def on_bar(self, df: pd.DataFrame, position: PositionSide = None) -> Dict:
        ...
        signal = self._check_entries(df, current, position)
    ...
    def _detect_bullish_bos(self, df: pd.DataFrame, current, current_idx: int):
        ...
        if not self.short_obs:
            return
        created_bullish = False
        for i in range(len(self.short_obs) - 1, -1, -1):
            short_ob = self.short_obs[i]
            if current['close'] > short_ob.top and current_idx > short_ob.index:
                self.bullish_alert = True
                if not created_bullish and current_idx - self.last_down_index < self.max_age_bars and current_idx > self.last_long_index:
                    ob = OrderBlock(
    ...
    def _update_ob_status(self, df: pd.DataFrame, current, current_idx: int):
        """Update OB status (mitigation checks) — reverse iteration matches PineScript"""
        for i in range(len(self.long_obs) - 1, -1, -1):
            ob = self.long_obs[i]
